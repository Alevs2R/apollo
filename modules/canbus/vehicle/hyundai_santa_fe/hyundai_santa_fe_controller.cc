...
ErrorCode HyundaiSantaFeController::Init(
    const VehicleParameter &params,
    CanSender<::apollo::canbus::ChassisDetail> *const can_sender,
    MessageManager<::apollo::canbus::ChassisDetail> *const message_manager) {
  if (is_initialized_) {
    AINFO << "HyundaiSantaFeController has already been initiated.";
  ...
  if (brake_60_ == nullptr) {
    AERROR << "Brake60 does not exist in the HyundaiSantaFeMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (throttle_62_ == nullptr) {
    AERROR << "Throttle62 does not exist in the HyundaiSantaFeMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (steering_64_ == nullptr) {
    AERROR << "Steering64 does not exist in the HyundaiSantaFeMessageManager!";
    return ErrorCode::CANBUS_ERROR;
  }
...
bool HyundaiSantaFeController::Start() {
  if (!is_initialized_) {
    AERROR << "HyundaiSantaFeController has NOT been initiated.";
    return false;
  }
...
void HyundaiSantaFeController::Stop() {
  if (!is_initialized_) {
    AERROR << "HyundaiSantaFeController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "HyundaiSantaFeController stopped.";
  }
}

Chassis HyundaiSantaFeController::chassis() {
  chassis_.Clear();
  ...
  // 7
  // hyundai_santa_fe only has fuel percentage
  ...
  //     chassis_detail.light().has_hyundai_santa_fe_lamp_type()) {
  //   chassis_.mutable_signal()->set_high_beam(
  //       chassis_detail.light().hyundai_santa_fe_lamp_type() == Light::BEAM_HIGH);
  // } else {
  ...
  // 19, hyundai_santa_fe wiper is too complicated
  // 24
...
void HyundaiSantaFeController::Emergency() {
...
ErrorCode HyundaiSantaFeController::EnableAutoMode() {
...
ErrorCode HyundaiSantaFeController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode HyundaiSantaFeController::EnableSteeringOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
ErrorCode HyundaiSantaFeController::EnableSpeedOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
void HyundaiSantaFeController::Gear(Chassis::GearPosition ref_gear_position) {
  // if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
  //     driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
// -> pedal
void HyundaiSantaFeController::Brake(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
...
// gas:0.00~99.99 unit:%
void HyundaiSantaFeController::Throttle(double pedal) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
...
void HyundaiSantaFeController::Acceleration(double acc) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_SPEED_ONLY) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
...
// hyundai_santa_fe default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
void HyundaiSantaFeController::Steer(double angle) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
...
void HyundaiSantaFeController::Steer(double angle, double angle_spd) {
  if (driving_mode() != Chassis::COMPLETE_AUTO_DRIVE &&
      driving_mode() != Chassis::AUTO_STEER_ONLY) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
...
void HyundaiSantaFeController::SetEpbBreak(const ControlCommand &command) {
...
void HyundaiSantaFeController::SetBeam(const ControlCommand &command) {
...
void HyundaiSantaFeController::SetHorn(const ControlCommand &command) {
...
void HyundaiSantaFeController::SetTurningSignal(const ControlCommand &command) {
  // Set Turn Signal
...
void HyundaiSantaFeController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool HyundaiSantaFeController::CheckChassisError() {
  ChassisDetail chassis_detail;
  message_manager_->GetSensorData(&chassis_detail);
...
void HyundaiSantaFeController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR_EVERY(100)
          << "Too much time consumption in HyundaiSantaFeController looping process:"
          << elapsed.count();
      start = end;
    }
  }
...
bool HyundaiSantaFeController::CheckResponse(const int32_t flags, bool need_wait) {
  // for HyundaiSantaFe, CheckResponse commonly takes 300ms. We leave a 100ms buffer
  // for it.
...
void HyundaiSantaFeController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t HyundaiSantaFeController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode HyundaiSantaFeController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void HyundaiSantaFeController::set_chassis_error_code(
    const Chassis::ErrorCode &error_code) {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  chassis_error_code_ = error_code;
}

bool HyundaiSantaFeController::CheckSafetyError(
